...
func TestTranslatePlainMD(t *testing.T) {
	...
	defer srv.Close()
	cmd := &TranslateCmd{
		File: []string{in}, From: "English", To: "French", Token: "tok", clientBase: srv.URL, Instructions: style}
...
func TestTranslateOutputExistsRefused(t *testing.T) {
	...
	cmd := &TranslateCmd{
		File: []string{in}, Output: out, Token: "tok"}
	...
	defer srv.Close()
	cmd = &TranslateCmd{
		File: []string{in}, Output: out, Force: true, Token: "tok", clientBase: srv.URL}
	if err := cmd.Run(nil, context.Background()); err != nil {
...
func TestTranslateLRCVerificationRetry(t *testing.T) {
	...
	defer srv.Close()
	cmd := &TranslateCmd{
		File: []string{in}, To: "French", Token: "tok", clientBase: srv.URL}
	if err := cmd.Run(nil, context.Background()); err != nil {
		t.Fatalf("translate: %v", err)
	}
